    ...
    <div class="card-body">
        <form method="post" action="{{ route('mod.tai_khoan.thiet_lap.cap_nhat_chi_tiet') }}">
            ...
            <div class="form-group">
                ...
                <input id="name" type="text" name="name" class="form-control" value="{{ Auth::user()->name }}" maxlength="{{ tbl_fields['user']['name'] }}" autocomplete="off" required>
            </div>
            <div class="form-group">
                <label for="email_address">Địa chỉ email</label>
                <input id="email_address" type="email" name="email" class="form-control" value="{{ Auth::user()->email }}" maxlength="{{ tbl_fields['user']['email'] }}" autocomplete="off" required>
            </div>
            <div class="form-group">
